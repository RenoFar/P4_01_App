...
def create_tournament():  # create the tournament
    ...
    while len(name) < 1 or not name.isalnum():
        name = InputView.input_data('Please enter the tournament name: ', '\n')
    while len(place) < 1:
        place = InputView.input_data('Please enter the tournament place: ')
    while len(date) < 1:
        date = InputView.input_data('Tournament date: ')
    description = InputView.input_data('Please enter the tournament description: ')
    while True:
        nb_turn = InputView.input_data('The number of laps by default is 4, '
    ...
    while game_mode.lower() not in ('bullet', 'blitz', 'speed'):
        game_mode = InputView.input_data('Please enter the tournament mode (bullet / blitz / speed): ')
